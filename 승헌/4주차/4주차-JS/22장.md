...
#### 22.1 this 키워드
---
객체는 상태를 나타내느 프로퍼티와 동작을 나타내느 메서드를 하나의 논리적인 단위로 묶은 복합적인 자료구조다.
...
이 참조표현식이 평가되는 시점은 getDiameter 메서드가 호출되어 함수 몸체가 실행되는 시점이다.
...
따라서 메서드 내부의 this는 프로퍼티로 메서드를 가리키고 있는 객체와는 관계가 없고 메서드를 호출할 객체에 바인딩된다.

프로토타입 메서드 내부에서 사용된 this도 일반 메서드와 마찬가지로 해당 메서드를 호출할 객체에 바인딩된다.
...
apply와 call 메서든 호출할 함수에 인수를 전달하는 방식만 다를 뿐 동일하게 동작한다.
...
이처럼 apply와 call 메서드는 호출할 함수에 인수를 전달하는 방식만 다를 뿐 this로 사용할 객체를 전달하면서 함수로 호출하는 것은 동일하다.
