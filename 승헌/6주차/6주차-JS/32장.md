...
String 래퍼 객체는 배열과 마찬가지로 length 프로퍼티와 인덱스를 나타내는 숫자 형식의 문자열을 프로퍼티 키로, 각 문자를 프로퍼티 값으로 갖는 유사 배열 객체이면서 이터러블이다 따라서 배열과 유사하게 인덱스를 사용하여 각 문제에 접근할 수 있다
...
new 연산자를 사용하지 않고 String 생성자 함수를 호출하면 String 인스터스가 아닌 문자열을 반환한다 이를 이용하여 명시적으로 타입을 반환하기도 한다
...
indexOf 메서드는 대상 문자열에서 인수로 전달받은 문자열을 검색하여 첫 번째 인덱스를 반환한다 검색에 실패하면 -1 반환한다
...
endWith 메서드의 2번째 인수로 검색할 문자열의 길이를 전달할 수 있다
...
인데스는 문자열의 범위, 즉 0 ~ (문자열 길이 - 1) 사이의 정수이어야 한다 인덱스가 문자열의 범위를 벗어난 정수인 경우 빈 문자열을 반환한다
...
charAt 메서드와 유사한 문자열 메서드는 String.prototype.charCodeAt고 String.prototype.codePointAt이 있다
...
substring 메서드의 두 번째 인수는 생략할 수 있다. 첫 번째 인수로 전달한 인덱스에 위치하는 문자부터 마지막 문자까지 부분 문자열을 반환한다
...
* 인수 > 문자열의 길이(str.length)인 경우 인수는 문자열의 길이(str.length)로 취급한다
...
slice 메서드는 substring 메서드와 동일하게 동작한다 음수인 인수를 전달하면 대상 문자열의 가장 뒤에서부터 시작하여 문자열을 잘라내어 반환한다
...
ES6에서 도입된 repeat 메서드는 대상 문자열을 인수로 전달받은 정수만큼 반복해 연결한 새로운 문자열을 반환한다 인수로 전달받은 정수가 0이면 빈 문자열을 반환하고 음수면 RangeError를 발생시킨다 인수를 생력하면 기본값 0이 설정된다
...
split 메서드는 대상 문자열에서 첫 번째 인수로 전달한 문자열 또는 정규 표현식을 검색하여 문자열 구분한 후 분리된 각 문자열로 이루어진 배열을 반환한다
