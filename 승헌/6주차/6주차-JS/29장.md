...
Math.floor 메서드는 인수로 전달된 숫자의 소수점 이하를 내림한 정수를 반환한다 즉 Math.ceil 메서드의 반대 개념이다 소수점 이하를 내림하면 더 작은 정수가 된다
...
Math.random 메서드는 임의의 난수를 반환한다 Math.random 메서드가 반환한 난수는 0에서 1미만의 실수다
