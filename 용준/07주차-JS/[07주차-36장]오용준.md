...
- ES5에서 구조화된 배열을 디스트럭처링하여 1개 이상의 변수에 할당하는 방법은 아래와 같음
...
- **이때 배열 디스트럭처링 할당의 대상(할당문의 우변)은 이터러블이어야 하며, 할당 기준은 배열의 인덱스임**
  - 즉, 순서대로 할당됨
...
- 배열 디스트럭처링 할당의 변수 선언문은 아래 코드처럼 선언과 할당을 분리할 수도 있음
- 하지만 이 경우 const 키워드로 변수를 선언할 수 없으므로 권장하지 않음
...
- URL을 파싱하여 protocol, host, path 프로퍼티를 갖는 객체를 생성해 반환하는 코드
...
- 배열 디스트럭처링과 할당과 마찬가지로 객체 디스트럭처링 할당을 위해서는 할당 연산자 왼쪽에 프로퍼티 값을 할당받을 변수를 선언해야 함
...
- 위 코드에서 객체 리터럴 형태로 선언한 변수는 lastName, firstName임. 이는 프로퍼티 축약 표현을 통해 선언한 것임
...
- 따라서 객체의 프로퍼티 키와 다른 변수 이름으로 프로퍼티 값을 할당받으려면 아래와 같이 변수를 선언함
...
- 위 코드에서 객체를 인수로 전달받는 매개변수 todo에 객체 디스트럭처링 할당을 사용하면 좀 더 간단하고 가독성 좋게 표현할 수 있음
...
- Rest 프로퍼티는 스프레드 프로퍼티외 함께 TC39 프로세스의 stage 4(Finished) 단계에 제안되어 있음
